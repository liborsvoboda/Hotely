        ...
        public async Task<string> GetReservedRoomsApi(int hotelId, DateTime startDate, DateTime endDate, string language = "cz") {
            ...
            try {
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    bookedCalendarMonthsCountBefore = int.Parse(new hotelsContext().WebPageSettingLists.Where(a => a.Key == "BookCalendarViewMonthsBefore").FirstOrDefault().Value);
                }
            ...
            try {
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                    bookedCalendarMonthsCountAfter = int.Parse(new hotelsContext().WebPageSettingLists.Where(a => a.Key == "BookCalendarViewMonthsAfter").FirstOrDefault().Value);
                }
