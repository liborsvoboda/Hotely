    ...
    public partial class TemplateClassListWithSubViewPage : UserControl {
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.BasicCurrencyList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.BasicCurrencyList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.BasicCurrencyList, httpContent, null, App.UserData.Authentification.Token); }
        ...
        public async void LoadSubDataList() {
            ...
            try {
                if (MainWindow.serviceRunning) exchangeRateList = await CommunicationManager.GetApiRequest<List<BusinessExchangeRateList>>(ApiUrls.BusinessExchangeRateList, null, App.UserData.Authentification.Token);
