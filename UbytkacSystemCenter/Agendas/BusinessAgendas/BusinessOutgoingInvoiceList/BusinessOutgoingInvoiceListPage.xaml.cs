    ...
    public partial class BusinessOutgoingInvoiceListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                ...
                DocumentAdviceList = await CommunicationManager.GetApiRequest<SystemDocumentAdviceList>(ApiUrls.SystemDocumentAdviceList, "outgoingInvoice/" + defaultAddress.Id, App.UserData.Authentification.Token);
                ...
                CurrencyList.ForEach(async currency => {
                    if (!currency.Fixed) {
                        BusinessExchangeRateList exchangeRate = await CommunicationManager.GetApiRequest<BusinessExchangeRateList>(ApiUrls.BusinessExchangeRateList, currency.Name, App.UserData.Authentification.Token);
                ...
                outgoingInvoiceList = await CommunicationManager.GetApiRequest<List<BusinessOutgoingInvoiceList>>(ApiUrls.BusinessOutgoingInvoiceList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.BusinessOutgoingInvoiceList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.BusinessOutgoingInvoiceList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.BusinessOutgoingInvoiceList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.RecordCount > 0) {
                    ...
                    dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.BusinessOutgoingInvoiceSupportList, dBResult.Status, App.UserData.Authentification.Token);
                    ...
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.BusinessOutgoingInvoiceSupportList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void SetRecord(bool? showForm = null, bool copy = false) {
            try {
                ...
                if (showForm != null && showForm == true) {
                    ...
                    else { DocumentItemList = await CommunicationManager.GetApiRequest<List<DocumentItemList>>(ApiUrls.BusinessOutgoingInvoiceSupportList, originalDocumentNumber, App.UserData.Authentification.Token); }
        ...
        private async void ImportOffer() {
            try {
                ...
                OutgoingInvoiceList = await CommunicationManager.GetApiRequest<List<BusinessOutgoingInvoiceList>>(ApiUrls.BusinessOutgoingInvoiceList, null, App.UserData.Authentification.Token);
                OutgoingInvoiceList.ForEach(record => {
        ...
        private async void ShowReceipt_Click(object sender, RoutedEventArgs e) {
            BusinessReceiptList receiptList = await CommunicationManager.GetApiRequest<BusinessReceiptList>(ApiUrls.BusinessReceiptList, selectedRecord.ReceiptId.ToString(), App.UserData.Authentification.Token);
        ...
        private async void ShowCreditNote_Click(object sender, RoutedEventArgs e) {
            BusinessCreditNoteList creditNoteList = await CommunicationManager.GetApiRequest<BusinessCreditNoteList>(ApiUrls.BusinessCreditNoteList, selectedRecord.CreditNoteId.ToString(), App.UserData.Authentification.Token);
