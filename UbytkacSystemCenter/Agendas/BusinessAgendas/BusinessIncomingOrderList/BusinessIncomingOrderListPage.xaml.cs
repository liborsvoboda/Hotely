    ...
    public partial class BusinessIncomingOrderListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                ...
                DocumentAdviceList = await CommunicationManager.GetApiRequest<SystemDocumentAdviceList>(ApiUrls.SystemDocumentAdviceList, "incomingOrder/" + defaultAddress.Id, App.UserData.Authentification.Token);
                ...
                IncomingOrderList = await CommunicationManager.GetApiRequest<List<BusinessIncomingOrderList>>(ApiUrls.BusinessIncomingOrderList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.BusinessIncomingOrderList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.BusinessIncomingOrderList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.BusinessIncomingOrderList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.RecordCount > 0) {
                    ...
                    dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.BusinessIncomingOrderSupportList, dBResult.Status, App.UserData.Authentification.Token);
                    ...
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.BusinessIncomingOrderSupportList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void SetRecord(bool? showForm = null, bool copy = false) {
            try {
                ...
                if (showForm != null && showForm == true) {
                    ...
                    else { DocumentItemList = await CommunicationManager.GetApiRequest<List<DocumentItemList>>(ApiUrls.BusinessIncomingOrderSupportList, originalDocumentNumber, App.UserData.Authentification.Token); }
        ...
        private async void SetSubListsNonActiveOnNewItem(bool newItem) {
            if (newItem) {
                AddressList = (await CommunicationManager.GetApiRequest<List<BusinessAddressList>>(ApiUrls.BusinessAddressList, null, App.UserData.Authentification.Token)).Where(a => a.Active && (a.AddressType == "all" || a.AddressType == "customer")).ToList();
                ...
                ItemList = (await CommunicationManager.GetApiRequest<List<BasicItemList>>(ApiUrls.BasicItemList, null, App.UserData.Authentification.Token)).Where(a => a.Active).ToList();
            }
            else {
                AddressList = (await CommunicationManager.GetApiRequest<List<BusinessAddressList>>(ApiUrls.BusinessAddressList, null, App.UserData.Authentification.Token)).Where(a => a.AddressType == "all" || a.AddressType == "customer").ToList();
        ...
        private async void ImportOffer() {
            try {
                ...
                IncomingOrderList = await CommunicationManager.GetApiRequest<List<BusinessIncomingOrderList>>(ApiUrls.BusinessIncomingOrderList, null, App.UserData.Authentification.Token);
                IncomingOrderList.ForEach(record => {
