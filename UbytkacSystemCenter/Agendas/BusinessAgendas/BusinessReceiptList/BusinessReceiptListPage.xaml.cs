    ...
    public partial class BusinessReceiptListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                ...
                DocumentAdviceList = await CommunicationManager.GetApiRequest<SystemDocumentAdviceList>(ApiUrls.SystemDocumentAdviceList, "receipt/" + defaultAddress.Id, App.UserData.Authentification.Token);
                ...
                ReceiptList = await CommunicationManager.GetApiRequest<List<BusinessReceiptList>>(ApiUrls.BusinessReceiptList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.BusinessReceiptList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.BusinessReceiptList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.BusinessReceiptList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.RecordCount > 0) {
                    ...
                    dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.BusinessReceiptSupportList, dBResult.Status, App.UserData.Authentification.Token);
                    ...
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.BusinessReceiptSupportList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void SetRecord(bool? showForm = null, bool copy = false) {
            ...
            if (showForm != null && showForm == true) {
                ...
                else { DocumentItemList = await CommunicationManager.GetApiRequest<List<DocumentItemList>>(ApiUrls.BusinessReceiptSupportList, originalDocumentNumber, App.UserData.Authentification.Token); }
        ...
        private async void SetSubListsNonActiveOnNewItem(bool newItem) {
            if (newItem) {
                ...
                AddressList = (await CommunicationManager.GetApiRequest<List<BusinessAddressList>>(ApiUrls.BusinessAddressList, null, App.UserData.Authentification.Token)).Where(a => a.Active && (a.AddressType == "all" || a.AddressType == "supplier")).ToList();
        ...
        private async void ImportInvoice() {
            try {
                ...
                ReceiptList = await CommunicationManager.GetApiRequest<List<BusinessReceiptList>>(ApiUrls.BusinessReceiptList, null, App.UserData.Authentification.Token);
                ReceiptList.ForEach(record => {
