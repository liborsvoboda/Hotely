...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class SystemDocumentAdviceListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                cb_branch.ItemsSource = branchList = await CommunicationManager.GetApiRequest<List<BusinessBranchList>>(ApiUrls.BusinessBranchList, null, App.UserData.Authentification.Token);
                documentAdviceList = await CommunicationManager.GetApiRequest<List<SystemDocumentAdviceList>>(ApiUrls.SystemDocumentAdviceList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
                documentTypeList = await CommunicationManager.GetApiRequest<List<SystemDocumentTypeList>>(ApiUrls.SystemDocumentTypeList, null, App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.SystemDocumentAdviceList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.SystemDocumentAdviceList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.SystemDocumentAdviceList, httpContent, null, App.UserData.Authentification.Token); }
