...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class HotelPropertyAndServiceListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                hotelPropertyAndServiceList = await CommunicationManager.GetApiRequest<List<HotelPropertyAndServiceList>>(ApiUrls.HotelPropertyAndServiceList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
                ...
                propertyOrServiceTypeList = await CommunicationManager.GetApiRequest<List<PropertyOrServiceTypeList>>(ApiUrls.PropertyOrServiceTypeList, null, App.UserData.Authentification.Token);
                propertyOrServiceUnitList = await CommunicationManager.GetApiRequest<List<PropertyOrServiceUnitList>>(ApiUrls.PropertyOrServiceUnitList, null, App.UserData.Authentification.Token);
                propertyGroupList = await CommunicationManager.GetApiRequest<List<PropertyGroupList>>(ApiUrls.PropertyGroupList, null, App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.HotelPropertyAndServiceList, httpContent, null, App.UserData.Authentification.Token);
                } else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.HotelPropertyAndServiceList, httpContent, null, App.UserData.Authentification.Token); }
