...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class PropertyGroupListPage : UserControl {
        public static DataViewSupport dataViewSupport = new DataViewSupport();
        public static PropertyGroupList selectedRecord = new PropertyGroupList();

        private List<PropertyGroupList> propertyGroupList = new List<PropertyGroupList>();

        public PropertyGroupListPage() {
            InitializeComponent();
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {  
                    propertyGroupList = await CommunicationManager.GetApiRequest<List<PropertyGroupList>>(ApiUrls.PropertyGroupList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);

                    propertyGroupList.ForEach(async item => { item.Translation = await DBOperations.DBTranslation(item.SystemName); });

                    DgListView.ItemsSource = propertyGroupList;
        ...
        public void Filter(string filter) {
            try {
                ...
                DgListView.Items.Filter = (e) => {
                    PropertyGroupList user = e as PropertyGroupList;
        ...
        public void NewRecord() {
            selectedRecord = new PropertyGroupList();
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(true);
        }

        public void EditRecord(bool copy) {
            selectedRecord = (PropertyGroupList)DgListView.SelectedItem;
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(true, copy);
        }
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.PropertyGroupList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private void DgListView_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            if (DgListView.SelectedItems.Count > 0) { selectedRecord = (PropertyGroupList)DgListView.SelectedItem; } else { selectedRecord = new PropertyGroupList(); }
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(false);
        }
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.PropertyGroupList, httpContent, null, App.UserData.Authentification.Token);
                } else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.PropertyGroupList, httpContent, null, App.UserData.Authentification.Token); }
        ...
        private void BtnCancel_Click(object sender, RoutedEventArgs e) {
            selectedRecord = (DgListView.SelectedItems.Count > 0) ? (PropertyGroupList)DgListView.SelectedItem : new PropertyGroupList();
            SetRecord(false);
        }
