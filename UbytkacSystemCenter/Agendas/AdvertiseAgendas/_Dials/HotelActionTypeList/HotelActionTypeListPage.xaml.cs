...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class HotelActionTypeListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                DgListView.ItemsSource = HotelActionTypeLists = await CommunicationManager.GetApiRequest<List<HotelActionTypeList>>(ApiUrls.HotelActionTypeList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
                HotelActionTypeLists.ForEach(async property => { property.Translation = await DBOperations.DBTranslation(property.SystemName); });
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.HotelActionTypeList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private void DgListView_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            if (DgListView.SelectedItems.Count > 0) { selectedRecord = (HotelActionTypeList)DgListView.SelectedItem; } else { selectedRecord = new HotelActionTypeList(); }
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(false);
        }
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.HotelActionTypeList, httpContent, null, App.UserData.Authentification.Token);
                } else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.HotelActionTypeList, httpContent, null, App.UserData.Authentification.Token); }
