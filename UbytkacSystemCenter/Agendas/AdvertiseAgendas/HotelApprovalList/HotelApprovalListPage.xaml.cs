...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class HotelApprovalListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                HotelApprovalList = await CommunicationManager.GetApiRequest<List<HotelApprovalList>>(ApiUrls.HotelApprovalList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
                ...
                basicCurrencyList = await CommunicationManager.GetApiRequest<List<BasicCurrencyList>>(ApiUrls.BasicCurrencyList, null, App.UserData.Authentification.Token);
                ...
                if (App.UserData.Authentification.Role == "admin") {
                    cb_owner.ItemsSource = adminUserList = await CommunicationManager.GetApiRequest<List<SolutionUserList>>(ApiUrls.SolutionUserList, null, App.UserData.Authentification.Token);
        ...
        private async void BtnSaveHotel_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                dBResult = await CommunicationManager.PostApiRequest(ApiUrls.HotelList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void CountrySelectionChanged(object sender, SelectionChangedEventArgs e) {
            if (cb_countryId.SelectedItem != null) {
                ...
                cityList = await CommunicationManager.GetApiRequest<List<CityList>>(ApiUrls.CityList, "ByCountry/" + ((CountryList)cb_countryId.SelectedItem).Id.ToString(), App.UserData.Authentification.Token); ;
        ...
        public async Task<bool> LoadRoomDataList() {
            ...
            try {
                if (hotelList.Id != 0) {
                    hotelRoomList = await CommunicationManager.GetApiRequest<List<HotelRoomList>>(ApiUrls.HotelApprovalList, "Rooms/" + hotelList.Id, App.UserData.Authentification.Token);
                    hotelRoomTypeList = await CommunicationManager.GetApiRequest<List<HotelRoomTypeList>>(ApiUrls.HotelRoomTypeList, null, App.UserData.Authentification.Token);
                    basicCurrencyList = await CommunicationManager.GetApiRequest<List<BasicCurrencyList>>(ApiUrls.BasicCurrencyList, null, App.UserData.Authentification.Token);
                    ...
                    //Only for Admin: Owner/UserId Selection
                    if (App.UserData.Authentification.Role == "admin") {
                        cb_owner1.ItemsSource = adminUserList = await CommunicationManager.GetApiRequest<List<SolutionUserList>>(ApiUrls.SolutionUserList, null, App.UserData.Authentification.Token);
        ...
        private async void BtnSaveRoom_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                StringContent httpContent = new StringContent(json, System.Text.Encoding.UTF8, "application/json");
                dBResult = await CommunicationManager.PostApiRequest(ApiUrls.HotelRoomList, httpContent, null, App.UserData.Authentification.Token);
