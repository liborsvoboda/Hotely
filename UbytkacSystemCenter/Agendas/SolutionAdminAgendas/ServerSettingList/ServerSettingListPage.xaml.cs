...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class ServerSettingListPage : UserControl {
        ...
        public async void LoadDataList() {
            ...
            try {
                serverLanguages = await CommunicationManager.GetApiRequest<List<SolutionMixedEnumList>>(ApiUrls.SolutionMixedEnumList, "ByGroup/ServerLanguages", App.UserData.Authentification.Token);
                serverConfigGroups = await CommunicationManager.GetApiRequest<List<SolutionMixedEnumList>>(ApiUrls.SolutionMixedEnumList, "ByGroup/ServerConfig", App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            ...
            if (PrepareConfigurationForSave()) {
                ...
                DBResultMessage dBResult = await CommunicationManager.PostApiRequest(ApiUrls.ServerSettingList, httpContent, null, App.UserData.Authentification.Token);
                MainWindow.ProgressRing = Visibility.Hidden;
        ...
        private async void BtnSendTestEmail_Click(object sender, RoutedEventArgs e) {
            try {
                SystemOperations.SendMailWithServerSetting(await DBOperations.DBTranslation("TestEmailFrom") + " " + App.ServerSetting.FirstOrDefault(a => a.Key == "ConfigCoreServerRegisteredName").Value);

                string dBResultMessage = await CommunicationManager.ApiManagerGetRequest(UrlSourceTypes.EicWebServerAuth, ApiUrls.ServerApi.ToString(), "ServerEmailer/" + (await DBOperations.DBTranslation("TestEmailFrom")) + " " + App.ServerSetting.FirstOrDefault(a => a.Key == "ConfigCoreServerRegisteredName").Value + " API");
                await MainWindow.ShowMessageOnMainWindow(false, dBResultMessage);
            } catch (Exception autoEx) { App.ApplicationLogging(autoEx); }
        }
