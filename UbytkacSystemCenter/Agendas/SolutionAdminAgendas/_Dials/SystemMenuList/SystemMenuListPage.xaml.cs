...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class SystemMenuListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                systemTableList = await CommunicationManager.GetApiRequest<List<GenericDataList>>(ApiUrls.StoredProceduresList, "DatabaseServices/SpGetSystemPageList", App.UserData.Authentification.Token);
                ...
                systemGroupMenuList = await CommunicationManager.GetApiRequest<List<SystemGroupMenuList>>(ApiUrls.SystemGroupMenuList, null, App.UserData.Authentification.Token);
                systemMenuList = await CommunicationManager.GetApiRequest<List<SystemMenuList>>(ApiUrls.SystemMenuList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.SystemMenuList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.SystemMenuList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.SystemMenuList, httpContent, null, App.UserData.Authentification.Token); }
