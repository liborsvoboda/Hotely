    ...
    public partial class SolutionSchedulerListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                ...
                solutionScheduledTypeList = await CommunicationManager.GetApiRequest<List<SolutionMixedEnumList>>(ApiUrls.SolutionMixedEnumList, "ByGroup/SchedulerDial", App.UserData.Authentification.Token);
                timeIntervalTypeList = await CommunicationManager.GetApiRequest<List<SolutionMixedEnumList>>(ApiUrls.SolutionMixedEnumList, "ByGroup/TimeIntervalDial", App.UserData.Authentification.Token);
                solutionSchedulerLists = await CommunicationManager.GetApiRequest<List<SolutionSchedulerList>>(ApiUrls.SolutionSchedulerList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommunicationManager.DeleteApiRequest(ApiUrls.SolutionSchedulerList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommunicationManager.PutApiRequest(ApiUrls.SolutionSchedulerList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommunicationManager.PostApiRequest(ApiUrls.SolutionSchedulerList, httpContent, null, App.UserData.Authentification.Token); }
        ...
        public async void LoadSubDataList() {
            ...
            try {
                ...
                solutionSchedulerProcessList = await CommunicationManager.GetApiRequest<List<SolutionSchedulerProcessList>>(ApiUrls.SolutionSchedulerProcessList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
