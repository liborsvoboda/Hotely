...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class MltTblGenEsbSrvFormPage : UserControl {
        ...
        private void MltTblGenEsbSrvFormPage_Loaded(object sender, RoutedEventArgs e) {
            ...
            if (systemCustomPageList.ShowHelpTab && systemCustomPageList.InheritedHelpTabSourceType.ToLower().Contains("helpurl")) { helpWebBrowser.EnsureCoreWebView2Async(App.appRuntimeData.WebViewEnvironment); }
        ...
        private async Task<bool> LoadInheritedDataList() {
            if (systemCustomPageList?.InheritedSetName?.Length > 0) {
                solutionMixedEnumList = await CommunicationManager.GetApiRequest<List<SolutionMixedEnumList>>(ApiUrls.SolutionMixedEnumList, "ByGroup/" + systemCustomPageList.InheritedSetName, App.UserData.Authentification.Token);
            } return true;
        }
    ...
    private async void TabSelectionChanged(object sender, SelectionChangedEventArgs e) {
            ...
            if (systemCustomPageList.DevModeEnabled) {
                App.ApplicationLogging(new Exception($"Data Updating on Tab Change: actual Command Result: " +
