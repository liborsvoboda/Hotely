...
namespace EasyITSystemCenter.GlobalOperations {
    ...
    internal class SystemOperations {
        ...
        public static void SendMailWithServerSetting(string message) {
            try {
                MailMessage Email = new MailMessage() { From = new MailAddress(App.ServerSetting.Find(a => a.Key == "EmailerSMTPLoginUsername").Value) };
                Email.To.Add(App.ServerSetting.Find(a => a.Key == "ConfigManagerEmailAddress").Value);
                Email.Subject = "Ubytkac Emailer";
                Email.Body = message;
                SmtpClient MailClient = new SmtpClient(App.ServerSetting.Find(a => a.Key == "EmailerSMTPServerAddress").Value, int.Parse(App.ServerSetting.Find(a => a.Key == "EmailerSMTPPort").Value))
                {
                    Credentials = new NetworkCredential(App.ServerSetting.Find(a => a.Key == "EmailerSMTPLoginUsername").Value, App.ServerSetting.Find(a => a.Key == "EmailerSMTPLoginPassword").Value),
                    EnableSsl = bool.Parse(App.ServerSetting.Find(a => a.Key == "EmailerSMTPSslIsEnabled").Value),
                    Host = App.ServerSetting.Find(a => a.Key == "EmailerSMTPServerAddress").Value,
