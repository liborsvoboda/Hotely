...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class InterestAreaListPage : UserControl {
        ...
        public InterestAreaListPage() {
            ...
            lbl_countryCity.Content = Resources["countryCity"].ToString();

            btn_saveCity.Content = Resources["btn_save"].ToString();
            btn_deleteCity.Content = Resources["delete"].ToString();
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                interestAreaList = await CommApi.GetApiRequest<List<InterestAreaList>>(ApiUrls.InterestAreaList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public void Filter(string filter) {
            try {
                ...
                DgListView.Items.Filter = (e) => {
                    InterestAreaList user = e as InterestAreaList;
                    return user.SystemName.ToLower().Contains(filter.ToLower())
                    || !string.IsNullOrEmpty(user.AreaTranslation) && user.AreaTranslation.ToLower().Contains(filter.ToLower())
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommApi.DeleteApiRequest(ApiUrls.InterestAreaList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommApi.PutApiRequest(ApiUrls.InterestAreaList, httpContent, null, App.UserData.Authentification.Token);
                } else { dBResult = await CommApi.PostApiRequest(ApiUrls.InterestAreaList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.RecordCount > 0) {
                    ...
                    dBResult = await CommApi.DeleteApiRequest(ApiUrls.InterestAreaCityList, dBResult.InsertedId.ToString(), App.UserData.Authentification.Token);
                    ...
                    dBResult = await CommApi.PutApiRequest(ApiUrls.InterestAreaCityList, httpContent, null, App.UserData.Authentification.Token);
        ...
        public async Task<bool> LoadSubDataList() {
            MainWindow.ProgressRing = Visibility.Visible;
            try {
                interestAreaCityList = await CommApi.GetApiRequest<List<InterestAreaCityList>>(ApiUrls.InterestAreaCityList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
