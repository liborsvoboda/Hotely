...
namespace EasyITSystemCenter.Pages {
    ...
    public partial class SolutionUserRoleListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                userRoleLists = await CommApi.GetApiRequest<List<SolutionUserRoleList>>(ApiUrls.SolutionUserRoleList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            if (result == MessageDialogResult.Affirmative) {
                DBResultMessage dBResult = await CommApi.DeleteApiRequest(ApiUrls.SolutionUserRoleList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                if (selectedRecord.Id == 0) {
                    dBResult = await CommApi.PutApiRequest(ApiUrls.SolutionUserRoleList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await CommApi.PostApiRequest(ApiUrls.SolutionUserRoleList, httpContent, null, App.UserData.Authentification.Token); }
