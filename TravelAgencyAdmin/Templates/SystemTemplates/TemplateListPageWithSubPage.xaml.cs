...
namespace TravelAgencyAdmin.Pages
{
    ...
    public partial class TemplateListPageWithSubPage : UserControl
    {
        ...
        public async Task<bool> LoadDataList()
        {
            ...
            try
            {
                defaultAddress = await ApiCommunication.GetApiRequest<BranchList>(ApiUrls.TravelAgencyAdminBranchList, "Active", App.UserData.Authentification.Token);
                DocumentAdviceList = await ApiCommunication.GetApiRequest<DocumentAdviceList>(ApiUrls.TravelAgencyAdminDocumentAdviceList, "offer/" + defaultAddress.Id, App.UserData.Authentification.Token);
                if (DocumentAdviceList == null) { await MainWindow.ShowMessage(true, Resources["documentAdviceNotSet"].ToString()); }
                cb_totalCurrency.ItemsSource = CurrencyList = await ApiCommunication.GetApiRequest<List<CurrencyList>>(ApiUrls.TravelAgencyAdminCurrencyList, null, App.UserData.Authentification.Token);
                cb_notes.ItemsSource = NotesList = await ApiCommunication.GetApiRequest<List<NotesList>>(ApiUrls.TravelAgencyAdminNotesList, null, App.UserData.Authentification.Token);
                cb_unit.ItemsSource = UnitList = await ApiCommunication.GetApiRequest<List<UnitList>>(ApiUrls.TravelAgencyAdminUnitList, null, App.UserData.Authentification.Token);
                cb_vat.ItemsSource = VatList = await ApiCommunication.GetApiRequest<List<VatList>>(ApiUrls.TravelAgencyAdminVatList, null, App.UserData.Authentification.Token);

                CurrencyList.ForEach(async currency => {
                    if (!currency.Fixed) { currency.ExchangeRate = (await ApiCommunication.GetApiRequest<ExchangeRateList>(ApiUrls.TravelAgencyAdminExchangeRateList, currency.Name, App.UserData.Authentification.Token)).Value; }
                });
                ...
                offerList = await ApiCommunication.GetApiRequest<List<OfferList>>(ApiUrls.TravelAgencyAdminOfferList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord()
        {
            ...
            {
                DBResultMessage dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.TravelAgencyAdminOfferList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                { dBResult = await ApiCommunication.PutApiRequest(ApiUrls.TravelAgencyAdminOfferList, httpContent, null, App.UserData.Authentification.Token);
                } else { dBResult = await ApiCommunication.PostApiRequest(ApiUrls.TravelAgencyAdminOfferList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.recordCount > 0)
                {
                    ...
                    dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.TravelAgencyAdminOfferItemList, dBResult.status, App.UserData.Authentification.Token);
                    ...
                    dBResult = await ApiCommunication.PutApiRequest(ApiUrls.TravelAgencyAdminOfferItemList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void SetRecord(bool showForm, bool copy = false)
        {
            ...
            if (showForm)
            {
                DocumentItemList = await ApiCommunication.GetApiRequest<List<DocumentItemList>>(ApiUrls.TravelAgencyAdminOfferItemList, originalDocumentNumber, App.UserData.Authentification.Token);
        ...
        private async void SetSubListsNonActiveOnNewItem(bool newItem)
        {
            if (newItem) {
                ...
                AddressList = (await ApiCommunication.GetApiRequest<List<AddressList>>(ApiUrls.TravelAgencyAdminAddressList, null, App.UserData.Authentification.Token)).Where(a => a.Active).ToList();
                ItemList = (await ApiCommunication.GetApiRequest<List<ItemList>>(ApiUrls.TravelAgencyAdminItemList, null, App.UserData.Authentification.Token)).Where(a => a.Active).ToList();
                ...
                AddressList = await ApiCommunication.GetApiRequest<List<AddressList>>(ApiUrls.TravelAgencyAdminAddressList, null, App.UserData.Authentification.Token);
                ItemList = await ApiCommunication.GetApiRequest<List<ItemList>>(ApiUrls.TravelAgencyAdminItemList, null, App.UserData.Authentification.Token);
