    ...
    internal class DBOperations {
        ...
        public static async void LoadStartupDBData() {
            try {
                ...
                App.IgnoredExceptionList = await CommApi.GetApiRequest<List<SystemIgnoredExceptionList>>(ApiUrls.SystemIgnoredExceptionList, null, null);
                App.LanguageList = await CommApi.GetApiRequest<List<SystemTranslationList>>(ApiUrls.SystemTranslationList, null, null);
                App.ParameterList = await CommApi.GetApiRequest<List<SystemParameterList>>(ApiUrls.SystemParameterList, null, null);
        ...
        public static async Task<bool> LoadWaitingDataInSleepMode() {
            if (App.ReloadLanguageListRequested) { App.LanguageList = await CommApi.GetApiRequest<List<SystemTranslationList>>(ApiUrls.SystemTranslationList, null, App.UserData.Authentification.Token); }
        ...
        public static async Task<bool> LoadOrRefreshUserData() {
            App.ServerSetting = await CommApi.GetApiRequest<List<ServerServerSettingList>>(ApiUrls.ServerSettingList, null, App.UserData.Authentification.Token);
            App.IgnoredExceptionList = await CommApi.GetApiRequest<List<SystemIgnoredExceptionList>>(ApiUrls.SystemIgnoredExceptionList, null, App.UserData.Authentification.Token);
            App.LanguageList = await CommApi.GetApiRequest<List<SystemTranslationList>>(ApiUrls.SystemTranslationList, null, App.UserData.Authentification.Token);
            App.ParameterList = await CommApi.GetApiRequest<List<SystemParameterList>>(ApiUrls.SystemParameterList, App.UserData.Authentification.Id.ToString(), App.UserData.Authentification.Token);
        ...
        public static async void SaveSystemFailMessage(string message, string logLevel) {
            if (MainWindow.ServiceRunning && MainWindow.UserLogged) {
                ...
                await CommApi.PutApiRequest(ApiUrls.SolutionFailList, httpContent, null, App.UserData.Authentification.Token);
            }
        }
                        ...
                        if (!string.IsNullOrWhiteSpace(word)) {
                            if (!App.ReloadLanguageListRequested && App.LanguageList.Where(a => a.SystemName.ToLower() == word.ToLower()).Count() == 0) {
                                if (!notCreateNew && App.UserData.Authentification != null) {
                                    ...
                                    StringContent httpContent = new StringContent(json, Encoding.UTF8, "application/json");
                                    await CommApi.PutApiRequest(ApiUrls.SystemTranslationList, httpContent, null, App.UserData.Authentification.Token);
                                }
                                result += word + ",";
                            }
                            else {
