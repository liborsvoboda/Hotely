    ...
    public partial class AdminLoginHistoryListPage : UserControl {
        ...
        private void DgListView_Translate(object sender, EventArgs ex) {
            ((DataGrid)sender).Columns.ToList().ForEach(e => {
                ...
                if (headername == "IpAddress") { e.Header = Resources["ipAddress"].ToString(); } else if (headername == "UserName") e.Header = Resources["userName"].ToString();
        ...
        private void DgListView_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            if (DgListView.SelectedItems.Count > 0) { selectedRecord = (AdminLoginHistoryList)DgListView.SelectedItem; } else { selectedRecord = new AdminLoginHistoryList(); }
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord();
        }
