...
namespace UbytkacAdmin.Pages {
    ...
    public partial class AccessRoleListPage : UserControl {
        ...
        private void DgListView_Translate(object sender, EventArgs ex) {
            try {
                ((DataGrid)sender).Columns.ToList().ForEach(e => {
                    ...
                    if (headername == "PageTranslation") { e.Header = Resources["tableName"].ToString(); e.DisplayIndex = 1; } else if (headername == "AccessRole") e.Header = Resources["accessRole"].ToString();
        ...
        private void DgListView_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            if (DgListView.SelectedItems.Count > 0) { selectedRecord = (AccessRoleList)DgListView.SelectedItem; } else { selectedRecord = new AccessRoleList(); }
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(false);
        }
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try {
                ...
                for (int i = 0; i < cb_accessRole.SelectedItems.Count; i++) { selectedRecord.AccessRole += ((UserRoleList)cb_accessRole.SelectedItems[i]).SystemName + ","; }
