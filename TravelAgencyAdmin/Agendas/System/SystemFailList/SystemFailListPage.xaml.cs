...
namespace TravelAgencyAdmin.Pages {
    ...
    public partial class SystemFailListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try { if (MainWindow.serviceRunning) DgListView.ItemsSource = await ApiCommunication.GetApiRequest<List<SystemFailList>>(ApiUrls.SystemFailList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token); } catch (Exception autoEx) { App.ApplicationLogging(autoEx); }
        ...
        private void DgListView_Translate(object sender, EventArgs ex) {
            try {
                ((DataGrid)sender).Columns.ToList().ForEach(e => {
                    ...
                    if (headername == "UserName") { e.Header = Resources["userName"].ToString(); e.DisplayIndex = 2; } else if (headername == "UserId") { e.Header = Resources["userId"].ToString(); e.DisplayIndex = 1; } else if (headername == "Message") e.Header = Resources["description"].ToString();
        ...
        private void DgListView_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            if (DgListView.SelectedItems.Count > 0) { selectedRecord = (SystemFailList)DgListView.SelectedItem; } else { selectedRecord = new SystemFailList(); }
            dataViewSupport.SelectedRecordId = selectedRecord.Id;
            SetRecord(false);
        }
