...
namespace TravelAgencyAdmin.Pages {
    ...
    public partial class InterestAreaListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try {
                interestAreaList = await ApiCommunication.GetApiRequest<List<InterestAreaList>>(ApiUrls.InterestAreaList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async Task<bool> LoadSubDataList() {
            MainWindow.ProgressRing = Visibility.Visible;
            try {
                interestAreaCityList = await ApiCommunication.GetApiRequest<List<InterestAreaCityList>>(ApiUrls.InterestAreaCityList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
