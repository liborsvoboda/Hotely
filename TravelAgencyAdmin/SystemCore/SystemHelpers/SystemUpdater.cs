...
namespace TravelAgencyAdmin.Helper
{
    public class SystemUpdater
    {
        ...
        public static async void CheckUpdate(bool directUpdate)
        {
            ...
            if (string.IsNullOrWhiteSpace(updateResult.Item2) && directUpdate) { await metroWindow.ShowMessageAsync(metroWindow.Resources["info"].ToString(), metroWindow.Resources["noneUpdateAvailable"].ToString(), MessageDialogStyle.Affirmative); }
            else if (updateResult.Item2 == "update")
            {
                ...
                else if (App.Setting.AutomaticUpdate == "automaticDownload") { DownloadUpdate(updateResult.Item1.ToString(), false); }
                else if (App.Setting.AutomaticUpdate == "automaticInstall") { DownloadUpdate(updateResult.Item1.ToString(), true); }
            }
        }
        ...
        private static async Task<Tuple<Version, string>> LastUpdate()
        {
            try
            {
                ...
                {
                    if (theLine.Split('_')[0].ToLower() == App.appName.ToLower() && Version.Parse(theLine.Split('_')[1].Remove(theLine.Split('_')[1].Length - 4, 4)) > updateVersion)
                    { updateVersion = Version.Parse(theLine.Split('_')[1].Remove(theLine.Split('_')[1].Length - 4, 4)); }
                }

                if (updateVersion > App.AppVersion) { return new Tuple<Version, string>(updateVersion, "update"); }
        ...
        private static async void DownloadCompleted(string version, bool installNow)
        {
            ...
            try
            {
                ...
                {
                    if (installNow) { InstallUpdate(Path.Combine(App.updateFolder, App.appName + "_" + version + ".exe")); }
                    else
                    {
                        ...
                        if (result == MessageDialogResult.Affirmative) { InstallUpdate(Path.Combine(App.updateFolder, App.appName + "_" + version + ".exe")); }
                        else { MainWindow.ProgressRing = Visibility.Hidden; }
                    }
                } else
                {   //Update file is under 1MB go next without info
