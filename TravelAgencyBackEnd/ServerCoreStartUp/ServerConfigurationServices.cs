...
using UbytkacBackend.ServerCoreServers;
...
namespace UbytkacBackend.ServerCoreConfiguration {
    ...
    public class ServerConfigurationServices {
        ...
        internal static void ConfigureDatabaseContext(ref IServiceCollection services) {
            try {
                services.AddDbContext<hotelsContext>(opt => opt.UseSqlServer("Server=95.183.52.33;Database=hotels;User ID=sa;Password=Hotel2023+;").UseQueryTrackingBehavior(QueryTrackingBehavior.NoTracking));
                //services.AddDbContext<Context>(options => options.UseFileContextDatabase(databaseName: "LocalDatabase",location: @"C:\Users\mjanatzek\Documents\Projects\test",password: "UbytkacBackend"));
            } catch (Exception ex) { }
        }
    }
}