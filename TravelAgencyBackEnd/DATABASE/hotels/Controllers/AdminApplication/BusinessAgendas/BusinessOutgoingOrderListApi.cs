namespace UbytkacBackend.Controllers {
    ...
    [Authorize]
    [ApiController]
    [Route("BusinessOutgoingOrderList")]
    public class BusinessOutgoingOrderListApi : ControllerBase {

        [HttpGet("/BusinessOutgoingOrderList")]
        public async Task<string> GetBusinessOutgoingOrderList() {
            List<BusinessOutgoingOrderList> data;
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                IsolationLevel = IsolationLevel.ReadUncommitted //with NO LOCK
            })) {
                data = new hotelsContext().BusinessOutgoingOrderLists.ToList();
            }

            return JsonSerializer.Serialize(data);
        }

        [HttpGet("/BusinessOutgoingOrderList/Filter/{filter}")]
        public async Task<string> GetBusinessOutgoingOrderListByFilter(string filter) {
            List<BusinessOutgoingOrderList> data;
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                IsolationLevel = IsolationLevel.ReadUncommitted //with NO LOCK
            })) {
                data = new hotelsContext().BusinessOutgoingOrderLists.FromSqlRaw("SELECT * FROM BusinessOutgoingOrderList WHERE 1=1 AND " + filter.Replace("+", " ")).AsNoTracking().ToList();
            }

            return JsonSerializer.Serialize(data);
        }

        [HttpGet("/BusinessOutgoingOrderList/{id}")]
        public async Task<string> GetBusinessOutgoingOrderListKey(int id) {
            BusinessOutgoingOrderList data;
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                IsolationLevel = IsolationLevel.ReadUncommitted
            })) {
                data = new hotelsContext().BusinessOutgoingOrderLists.Where(a => a.Id == id).First();
            }

            return JsonSerializer.Serialize(data);
        }

        [HttpPut("/BusinessOutgoingOrderList")]
        ...
        public async Task<string> InsertBusinessOutgoingOrderList([FromBody] BusinessOutgoingOrderList record) {
            try {
                ...
                documentAdvice = new hotelsContext().SystemDocumentAdviceLists.Where(a => a.DocumentType == "outgoingOrder" && (a.StartDate == null || a.StartDate <= DateTime.UtcNow.Date) && (a.EndDate == null || a.EndDate >= DateTime.UtcNow.Date)).FirstOrDefault();
                ...
                var data = new hotelsContext().BusinessOutgoingOrderLists.Add(record);
        ...
        [HttpPost("/BusinessOutgoingOrderList")]
        [Consumes("application/json")]
        public async Task<string> UpdateBusinessOutgoingOrderList([FromBody] BusinessOutgoingOrderList record) {
            try {
                var data = new hotelsContext().BusinessOutgoingOrderLists.Update(record);
                int result = await data.Context.SaveChangesAsync();
        ...
        [HttpDelete("/BusinessOutgoingOrderList/{id}")]
        [Consumes("application/json")]
        public async Task<string> DeleteBusinessOutgoingOrderList(string id) {
            try {
                ...
                docNumber = new hotelsContext().BusinessOutgoingOrderLists.First(a => a.Id == int.Parse(id)).DocumentNumber;
                BusinessOutgoingOrderList record = new() { Id = int.Parse(id), DocumentNumber = docNumber };

                var data = new hotelsContext().BusinessOutgoingOrderLists.Remove(record);
                int result = await data.Context.SaveChangesAsync();
