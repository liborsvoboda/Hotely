    ...
    public class DocumentAdviceListApi : ControllerBase {
        ...
        public async Task<string> GetDocumentAdviceListByFilter(string filter) {
            ...
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                ...
                if (Request.HttpContext.User.IsInRole("Admin".ToLower())) { data = new hotelsContext().DocumentAdviceLists.FromSqlRaw("SELECT * FROM DocumentAdviceList WHERE 1=1 AND " + filter.Replace("+", " ")).AsNoTracking().ToList(); }
                else {
