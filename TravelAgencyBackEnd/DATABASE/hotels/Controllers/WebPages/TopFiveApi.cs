...
namespace UbytkacBackend.Controllers {
    ...
    public class TopFiveApi : ControllerBase {
        ...
        [Authorize]
        [HttpGet("/WebApi/Guest/GetAuthTopFiveList/{type}/{count}/{language}")]
        public async Task<string> GetAuthTopFiveList(string type, int count, string language = "cz") {
                ...
                case "newest":
                    using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                        data = new hotelsContext().HotelLists.Where(a => a.Approved && a.Advertised && !a.Deactivated)
                            .OrderByDescending(a => a.Timestamp).Select(a => a.Id).Take(count).ToList();
                    }
                    break;
                case "cheapest":
                    using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                        data = new hotelsContext().HotelRoomLists
                            ...
                            .OrderBy(a => a.Price).Select(a => a.HotelId).Distinct().Take(count).ToList();
                    }
                    break;
                case "favoritest":
                    using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                        data = new hotelsContext().GetTopFiveFavoriteLists.Select(a => a.Id).Take(count).ToList();
                    }
                    break;
                case "bestreview":
                    using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                        data = new hotelsContext().HotelLists.Where(a => a.Approved && a.Advertised && !a.Deactivated)
                            .OrderByDescending(a => a.AverageRatingAmount).Take(count * 5).ToList()
                            .OrderByDescending(a => a.AverageRating).ToList()
                            .Select(a => a.Id).Take(count).ToList();
                    }
                    break;
        ...
        public async Task<string> GetTopFiveList(string type, string language = "cz") {
            ...
            switch (type) {
                ...
                case "favoritest":
                    using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                        data = new hotelsContext().GetTopFiveFavoriteLists.Select(a=> a.Id).Take(5).ToList();
                    }
                    break;
                case "bestreview":
                    using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted })) {
                        data = new hotelsContext().HotelLists.Where(a => a.Approved && a.Advertised && !a.Deactivated)
                            .OrderByDescending(a => a.AverageRatingAmount).Take(25).ToList()
                            .OrderByDescending(a => a.AverageRating).ToList()
                            .Select(a => a.Id).Take(5).ToList();
                    }
                    break;
