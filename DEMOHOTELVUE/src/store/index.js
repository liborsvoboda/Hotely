...
const store = createStore({
  ...
  actions: {
    ...
    async searchHotels({ commit }, searchString) {
      ...
      if (searchString === null || searchString == '') {
        var response = await fetch(
          'http://localhost:7000/api/Search/search?startDate=' +
            startDate +
            '&endDate=' +
            endDate +
            '&rooms=' +
        ...
        var response = await fetch(
          'http://localhost:7000/api/Search/search?input=' + searchString
        )
    ...
    async getHotelById({ commit }, hotelId) {
      var response = await fetch(
        'http://localhost:7000/api/Hotel/GetById/' + hotelId
      ) // Default is GET
      var result = await response.json()
    ...
    async getReservationById({ commit }, reservationId) {
      var response = await fetch(
        'http://localhost:7000/api/Booking/' + reservationId
      )
      var result = await response.json()
    ...
    async searchHotelByCity({ commit }, searchString) {
      var response = await fetch(
        'http://localhost:7000/api/Search/GetHotelByCity?input=' +
          searchString
      ) // Default is GET
      var result = await response.json()
      if (result) {
    ...
    async getAutoComplete({ commit }) {
      var response = await fetch(
        'http://localhost:7000/api/Search/GetSearchAutoComplete'
      )
      var result = await response.json()
      commit('setAutoComplete', result)
    },
    async login({ commit }, credentials) {
      let response = await fetch('http://localhost:7000/api/Guest/login', {
        method: 'post',
    ...
    async getSavedHotelsInfo({ commit }) {
      var response = await fetch(
        'http://localhost:7000/api/Hotel/SavedHotelsInfo?id=' +
          this.state.user.id
      )
      var result = await response.json()
      if (result) {
    ...
    async addFavouriteHotel({ commit }, hotelId) {
      fetch('http://localhost:7000/api/Guest/saveFavoriteHotel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    async removeFavouriteHotel({ commit }, hotelId) {
      fetch('http://localhost:7000/api/Guest/removeFavoriteHotel', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ hotelID: hotelId, guestID: this.state.user.id }),
      })
    ...
    async getReviews({ commit }, hotelId) {
      var response = await fetch(
        'http://localhost:7000/api/Hotel/GetReviews/' + hotelId
      )
      var result = await response.json()
    ...
    async setServiceFee({ commit }, payload) {
      var response = await fetch(
        'http://localhost:7000/api/Hotel/GetAccomodationFee?id=' +
          payload.id +
          '&type=' +
          payload.type
      )
    ...
    deleteGuestAccount({ commit }, Id) {
      fetch('http://localhost:7000/api/Guest/removeGuest', {
        method: 'post',
        headers: { 'Content-type': 'application/json' },
    ...
    async getBookings({ commit }) {
      var response = await fetch(
        'http://localhost:7000/api/Booking/guest/' + this.state.user.id
      )
      var result = await response.json()
